...
D3D12ComputeBasic::D3D12ComputeBasic(TCHAR const* const title, int width, int height) : Application(title, width, height)
{
...
{
    if (!physika::core::Application::Initialize()) {
        return false;
    }
...
void D3D12ComputeBasic::InitializeResources()
{
    ...
    mVectorOfNumbers1 = graphics::CreateDefaultBuffer(mD3D12Device, mGraphicsCommandList, numbersToAdd1.data(), byteSize);

    mVectorOfNumbers2 = graphics::CreateDefaultBuffer(mD3D12Device, mGraphicsCommandList, numbersToAdd2.data(), byteSize);

    mVectorAddResult = graphics::CreateOutputBuffer(mD3D12Device, byteSize);
}
...
void D3D12ComputeBasic::InitializePSOs()
{
    ...
    graphics::ID3DBlobPtr csByteCode = nullptr;
    ...
    graphics::ThrowIfFailed(mD3D12Device->CreateComputePipelineState(&psoDesc, IID_PPV_ARGS(&mPipelineState)));
}
